    ...
    decoded_text (
        ...
        do
            ...
            if code_page.is_case_insensitive_equal ({CODE_PAGE_CONSTANTS}.utf8) then
                ...
                if
                    source_encoding.last_conversion_successful and then
                    not source_encoding.last_conversion_lost_data
                then
                    converted_bytes := source_encoding.last_converted_stream.twin
                    if {UTF_CONVERTER}.is_valid_utf_8_string_8 (converted_bytes) then
                        converted_text := {UTF_CONVERTER}.utf_8_string_8_to_string_32 (
                            converted_bytes
                        )
                    else
                        conversion_failed := True
