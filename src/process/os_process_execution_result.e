...
feature {NONE} -- Initialization
	...
	make (a_was_launched: BOOLEAN; a_has_exit_code: BOOLEAN; a_exit_code: INTEGER; a_stdout: READABLE_STRING_8; a_stderr: READABLE_STRING_8; a_failures: ITERABLE [OS_PROCESS_FAILURE])
...
feature -- Status report
	...
	successful: BOOLEAN
		...
		do
			Result := was_launched and then has_exit_code and then exit_code_storage = 0 and then not has_failures
		ensure
			definition: Result = (was_launched and then has_exit_code and then exit_code = 0 and then not has_failures)
		end
