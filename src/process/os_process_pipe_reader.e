...
feature {NONE} -- Initialization
	...
	make (a_process: POINTER; a_is_stdout: BOOLEAN; a_handler: detachable PROCEDURE [READABLE_STRING_8])
...
feature {NONE} -- Reading
	...
	call_handler (a_chunk: READABLE_STRING_8)
		...
		local
			retried: BOOLEAN
		do
			if not retried and then not handler_has_failed and then attached handler as output_handler then
	...
	record_read_failure (a_native_code: INTEGER)
		...
		do
			...
			if a_native_code > 0 then
				create new_failure.make_with_native_code (failure_kind, operation_name, message, a_native_code)
			else
