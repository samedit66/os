...
feature {NONE} -- Writing
	...
	record_write_failure (a_native_code: INTEGER)
		...
		do
			...
			if a_native_code > 0 then
				create new_failure.make_with_native_code (failure_kind, "write stdin", "Cannot write standard input", a_native_code)
			else
				create new_failure.make (failure_kind, "write stdin", "Cannot write standard input")
			end
			failure_mutex.lock
	...
	record_close_failure (a_native_code: INTEGER)
		...
		do
			...
			create new_failure.make_with_native_code (failure_kind, "close stdin", "Cannot close standard input", a_native_code)
			failure_mutex.lock
